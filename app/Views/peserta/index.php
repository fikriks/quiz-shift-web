...
$(document).ready(function() {
    table = $('#pesertaTable').DataTable({
        "language": {
            "url": "https://cdn.datatables.net/plug-ins/1.10.24/i18n/Indonesian.json"
        },
        "columnDefs": [
            { "orderable": false, "targets": [7] } // disable sorting on aksi
        ],
...
function filterJenjang(jenjang) {
    ...
    if (jenjang === 'ALL') {
        ...
        if (btn) btn.classList.add('active');
        table.column(5).search('ELEMENTARY').draw();
    } else if (jenjang === 'HIGH_SCHOOL') {
        const btn = document.getElementById('tab-highschool');
        if (btn) btn.classList.add('active');
        table.column(5).search('HIGH SCHOOL').draw();
    }
