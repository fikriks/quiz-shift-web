...
Untuk mempermudah penulisan Bab 3 dan Bab 4 pada skripsi, telah dibuat **9 berkas diagram Draw.io (.drawio)** yang memiliki presisi tinggi, terstruktur per modul, memiliki pewarnaan bertema premium (Mantis Theme), serta menggunakan label dalam **Bahasa Indonesia Baku**. File-file ini tersimpan di dalam direktori `docs/` dan dapat diedit secara langsung menggunakan aplikasi [Draw.io](https://app.diagrams.net/):

1. **Use Case Diagram**: [use_case_s1.drawio](file:///Users/fikrikhairulshaleh/Valet/rian/quiz-shift-web/docs/use_case_s1.drawio)
2. **Activity Diagram - Login**: [activity_login_s1.drawio](file:///Users/fikrikhairulshaleh/Valet/rian/quiz-shift-web/docs/activity_login_s1.drawio)
3. **Activity Diagram - Kelola Level & Soal**: [activity_crud_level_soal_s1.drawio](file:///Users/fikrikhairulshaleh/Valet/rian/quiz-shift-web/docs/activity_crud_level_soal_s1.drawio)
4. **Activity Diagram - Pelaksanaan Ujian (Fisher-Yates)**: [activity_kuis_s1.drawio](file:///Users/fikrikhairulshaleh/Valet/rian/quiz-shift-web/docs/activity_kuis_s1.drawio)
5. **Entity Relationship Diagram (ERD)**: [erd_s1.drawio](file:///Users/fikrikhairulshaleh/Valet/rian/quiz-shift-web/docs/erd_s1.drawio)
6. **Class Diagram**: [class_diagram_s1.drawio](file:///Users/fikrikhairulshaleh/Valet/rian/quiz-shift-web/docs/class_diagram_s1.drawio)
7. **Sequence Diagram - Login**: [sequence_login_s1.drawio](file:///Users/fikrikhairulshaleh/Valet/rian/quiz-shift-web/docs/sequence_login_s1.drawio)
8. **Sequence Diagram - Kelola Soal (CRUD)**: [sequence_crud_level_soal_s1.drawio](file:///Users/fikrikhairulshaleh/Valet/rian/quiz-shift-web/docs/sequence_crud_level_soal_s1.drawio)
9. **Sequence Diagram - Pelaksanaan Kuis (Fisher-Yates)**: [sequence_kuis_s1.drawio](file:///Users/fikrikhairulshaleh/Valet/rian/quiz-shift-web/docs/sequence_kuis_s1.drawio)

> [!NOTE]
> Semua diagram di atas dibuat secara modular (tidak digabung menjadi satu diagram raksasa) untuk mempermudah pemaparan alur sistem di setiap sub-bab analisis dan perancangan skripsi.
...
- **Sub-bab 3.2: Pemodelan Fungsionalitas (Use Case)**
  - Masukkan gambar dari berkas `use_case_s1.drawio`. Jelaskan definisi aktor (Admin, Instruktur, Peserta) beserta hak akses masing-masing dalam bentuk tabel deskripsi use case.
- **Sub-bab 3.3: Pemodelan Dinamis (Activity Diagram)**
  - Gunakan `activity_login_s1.drawio`, `activity_crud_level_soal_s1.drawio`, dan `activity_kuis_s1.drawio` untuk merinci aktivitas pengguna pada setiap fungsionalitas sistem.
- **Sub-bab 3.4: Perancangan Struktur Data (ERD & Kamus Data)**
  - Salin struktur tabel MySQL lengkap yang ada di **Bagian 2** sebagai tabel kamus data fisik database Anda. Sertakan gambar dari berkas `erd_s1.drawio`.

### B. BAB IV - IMPLEMENTASI DAN PENGUJIAN SISTEM
- **Sub-bab 4.1: Arsitektur Perangkat Lunak (Class Diagram & Sequence)**
  - Tampilkan diagram `class_diagram_s1.drawio` untuk membuktikan implementasi OOP (Object-Oriented Programming) dan arsitektur MVC (Model-View-Controller).
  - Tampilkan diagram `sequence_login_s1.drawio`, `sequence_crud_level_soal_s1.drawio`, dan `sequence_kuis_s1.drawio` untuk menjelaskan urutan kronologis eksekusi fungsi controller, pemanggilan model, dan relasi database.
